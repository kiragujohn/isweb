...
export class EdituserComponent implements OnInit {
  ...
  private getEditUserInitialData(organisationId: number): void {
    this.adminEditUserService.getInitialData(organisationId).subscribe(
      (adminEditUserInitialData: AdminEditUserInitialData) => {
        this.adminEditUserInitialData = adminEditUserInitialData;
        if (adminEditUserInitialData.user.addressess[0] != null) {
          this.addressTypeId = adminEditUserInitialData.user.addressess[0].addressTypeId;
        }
      }
    );
  }
